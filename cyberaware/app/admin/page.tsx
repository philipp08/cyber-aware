...
const CustomBar = (props: any) => {
  ...
  return <rect x={x} y={y} width={width} height={height} fill={value >= 80 ? "#CCFF00" : value >= 50 ? "rgba(204,255,0,0.4)" : "rgba(204,255,0,0.15)"} rx={4} />;
};
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 16 }}>
          {[
            ...
            <div key={kpi.label} style={{ background: S.surface, border: `1px solid ${S.border}`, borderRadius: 18, padding: "1.25rem" }}>
              <div style={{ fontSize: "2rem", fontWeight: 900, letterSpacing: "-0.04em", lineHeight: 1, marginBottom: 4 }}>{kpi.value}</div>
              <div style={{ fontSize: "0.7rem", color: S.text2, marginBottom: 8 }}>{kpi.label}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 10, marginBottom: 16 }}>
          ...
          <div style={{ background: S.surface, border: `1px solid ${S.border}`, borderRadius: 18, padding: "1.5rem" }}>
            <div style={{ fontSize: "0.7rem", color: S.text2, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "1.25rem" }}>Abschlussquote nach Abteilung</div>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={completionData} margin={{ top: 0, right: 0, left: -24, bottom: 0 }}>
                <XAxis dataKey="dept" tick={{ fontSize: 10, fill: S.text2 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 10, fill: S.text2 }} axisLine={false} tickLine={false} domain={[0, 100]} />
                <Tooltip
                  contentStyle={{ background: S.surface2, border: `1px solid ${S.border}`, borderRadius: 10, color: S.text, fontSize: 12 }}
          ...
          {/* Cert distribution */}
          <div style={{ background: S.surface, border: `1px solid ${S.border}`, borderRadius: 18, padding: "1.5rem" }}>
            <div style={{ fontSize: "0.7rem", color: S.text2, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "1.25rem" }}>Zertifikatsverteilung</div>
              ...
              {[
                ...
                <div key={d.label}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: "0.75rem", color: S.text2 }}>{d.label}</span>
        ...
        <div style={{ background: S.surface, border: `1px solid ${S.border}`, borderRadius: 18, padding: "1.5rem", marginBottom: 16 }}>
          <div style={{ fontSize: "0.7rem", color: S.text2, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "1.25rem" }}>Zertifizierungstrend</div>
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={trendData} margin={{ top: 0, right: 10, left: -24, bottom: 0 }}>
              <XAxis dataKey="month" tick={{ fontSize: 10, fill: S.text2 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 10, fill: S.text2 }} axisLine={false} tickLine={false} />
              <Tooltip
                contentStyle={{ background: S.surface2, border: `1px solid ${S.border}`, borderRadius: 10, color: S.text, fontSize: 12 }}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
          ...
          <div style={{ background: S.surface, border: `1px solid ${S.border}`, borderRadius: 18, overflow: "hidden" }}>
            <div style={{ padding: "1.25rem 1.5rem", borderBottom: `1px solid ${S.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <Link href="/admin/users" style={{ fontSize: "0.68rem", color: S.accent, textDecoration: "none" }}>Alle →</Link>
            </div>
            {expiringCerts.length === 0 ? (
              <div style={{ padding: "2rem", textAlign: "center", fontSize: "0.78rem", color: S.text2 }}>Keine ablaufenden Zertifikate</div>
            ) : expiringCerts.map(e => {
              const days = Math.ceil((new Date(e.certExpiry!).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
              return (
                <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "0.85rem 1.5rem", borderBottom: `1px solid ${S.border}` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: "0.68rem", color: S.text2 }}>{e.department}</div>
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: "0.82rem", fontWeight: 700, color: days < 14 ? S.red : S.amber }}>{days}d</div>
                    <div style={{ fontSize: "0.65rem", color: S.text2 }}>{new Date(e.certExpiry!).toLocaleDateString("de-DE")}</div>
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ background: S.surface, border: `1px solid ${S.border}`, borderRadius: 18, overflow: "hidden" }}>
            <div style={{ padding: "1.25rem 1.5rem", borderBottom: `1px solid ${S.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <Link href="/admin/phishing" style={{ fontSize: "0.68rem", color: S.accent, textDecoration: "none" }}>Alle →</Link>
            </div>
              ...
              <div key={c.id} style={{ padding: "0.85rem 1.5rem", borderBottom: i < phishingCampaigns.length - 1 ? `1px solid ${S.border}` : "none" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
                  <div>
                    ...
                    <div style={{ fontSize: "0.68rem", color: S.text2, marginTop: 1 }}>{c.template}</div>
                  </div>
                ...
                {c.status !== "draft" && (
                  <div style={{ display: "flex", gap: 12, fontSize: "0.68rem", color: S.text2 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          {[
            ...
            <Link key={a.href} href={a.href} style={{ background: S.surface, border: `1px solid ${S.border}`, borderRadius: 18, padding: "1.25rem 1.5rem", textDecoration: "none", display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontSize: "1.5rem" }}>{a.emoji}</span>
              <div>
                <div style={{ fontSize: "0.85rem", fontWeight: 600, color: S.text, marginBottom: 2 }}>{a.label}</div>
                <div style={{ fontSize: "0.7rem", color: S.text2 }}>{a.desc}</div>
              </div>
            </Link>
          ))}
        </div>
