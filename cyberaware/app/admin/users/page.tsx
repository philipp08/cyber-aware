  ...
  return (
    <DashboardLayout isAdmin userName="Admin Müller" userInitials="AM" userRole="Unternehmens-Admin">
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          <div style={{ position: "relative", flex: 1, minWidth: 200 }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Mitarbeiter suchen..."
              style={{ width: "100%", background: "var(--surface)", border: `1px solid ${S.border}`, borderRadius: 100, padding: "0.55rem 1rem 0.55rem 2.4rem", fontSize: "0.82rem", color: "var(--text)", outline: "none" }} />
          </div>
        ...
        {/* Table */}
        <div style={{ background: "var(--surface)", border: `1px solid ${S.border}`, borderRadius: 18, overflow: "hidden", marginBottom: 12 }}>
          {/* Header */}
          ...
          {filtered.map((emp, i) => {
            ...
            const statusColor = emp.certLevel ? S.green : emp.completedModules > 0 ? "var(--accent-text)" : S.text3;
              ...
              <div key={emp.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 120px 100px 100px", gap: 0, padding: "0.85rem 1.5rem", borderBottom: i < filtered.length - 1 ? `1px solid ${S.border}` : "none", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: "0.68rem", color: "var(--text-2)" }}>{emp.email}</div>
                  </div>
                </div>
                <span style={{ fontSize: "0.78rem", color: "var(--text-2)" }}>{emp.department}</span>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ flex: 1, height: 3, background: "var(--surface-2)", borderRadius: 100, overflow: "hidden" }}>
                    <div style={{ width: `${progress}%`, height: "100%", background: progress === 100 ? S.green : S.accent, borderRadius: 100 }} />
                  </div>
                  <span style={{ fontSize: "0.65rem", color: "var(--text-2)", flexShrink: 0 }}>{emp.completedModules}/{emp.totalModules}</span>
                </div>
                <div>
                  {emp.certLevel ? (
                    <div>
                      <span style={{ fontSize: "0.68rem", fontWeight: 700, padding: "0.15rem 0.5rem", borderRadius: 100, background: emp.certLevel === "Gold" ? "var(--accent-dim2)" : "var(--border)", color: emp.certLevel === "Gold" ? "var(--accent-text)" : "var(--text-2)" }}>
                        {emp.certLevel}
                      </span>
        ...
        <div style={{ fontSize: "0.72rem", color: "var(--text-2)", marginBottom: 12 }}>{filtered.length} Einträge</div>

        {/* Bulk actions */}
        <div style={{ display: "flex", gap: 8 }}>
          <button style={{ display: "flex", alignItems: "center", gap: 6, padding: "0.5rem 1rem", background: "transparent", border: `1px solid ${S.border}`, borderRadius: 100, color: "var(--text-2)", fontSize: "0.75rem", cursor: "pointer" }}>
          ...
          <div style={{ background: "var(--surface)", border: `1px solid ${S.border}`, borderRadius: 24, padding: "2rem", width: "100%", maxWidth: 440 }}>
            {!inviteSent ? (
                ...
                <form onSubmit={handleInvite}>
                  {[
                    ...
                    <div key={f.label} style={{ marginBottom: "1rem" }}>
                      <label style={{ fontSize: "0.72rem", color: "var(--text-2)", display: "block", marginBottom: 6 }}>{f.label}</label>
                      <input type={f.type} value={f.value} onChange={e => f.onChange(e.target.value)} placeholder={f.placeholder} required={f.required}
                        style={{ width: "100%", background: "var(--surface-2)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "0.65rem 1rem", fontSize: "0.85rem", color: "var(--text)", outline: "none" }} />
                    </div>
                  ))}
                  <div style={{ marginBottom: "1rem" }}>
                    <label style={{ fontSize: "0.72rem", color: "var(--text-2)", display: "block", marginBottom: 6 }}>Abteilung</label>
                    <select style={{ width: "100%", background: "var(--surface-2)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "0.65rem 1rem", fontSize: "0.85rem", color: "var(--text)", outline: "none" }}>
                      {["IT", "Finance", "HR", "Marketing", "Sales", "Operations", "Legal"].map(d => <option key={d}>{d}</option>)}
                    </select>
                  </div>
                  <div style={{ marginBottom: "1rem" }}>
                    <label style={{ fontSize: "0.72rem", color: "var(--text-2)", display: "block", marginBottom: 6 }}>Lernpfad</label>
                    <select style={{ width: "100%", background: "var(--surface-2)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "0.65rem 1rem", fontSize: "0.85rem", color: "var(--text)", outline: "none" }}>
              ...
              <div style={{ textAlign: "center", padding: "2rem 0" }}>
                ...
                <div style={{ fontSize: "0.82rem", color: "var(--text-2)" }}>{inviteEmail}</div>
              </div>
            )}
          </div>
        </div>
