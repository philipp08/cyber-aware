      ...
      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        {phase === "intro" && (
          <>
            ...
            {/* Topics */}
            <div style={{ background: "var(--surface)", border: `1px solid ${S.border}`, borderRadius: 20, padding: "1.5rem", marginBottom: "1.5rem" }}>
            ...
            {currentUser.examAttempts > 0 && (
              <div style={{ background: "var(--surface)", border: `1px solid ${S.border}`, borderRadius: 20, padding: "1.25rem", marginBottom: "1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
            ...
            <button onClick={start} disabled={3 - currentUser.examAttempts <= 0}
              style={{ width: "100%", padding: "0.85rem", background: S.accent, color: "var(--accent-fg)", fontWeight: 700, fontSize: "0.9rem", border: "none", borderRadius: 100, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: 3 - currentUser.examAttempts <= 0 ? 0.4 : 1 }}>
        ...
        {phase === "exam" && q && (
          <>
            ...
            <div style={{ background: "var(--surface)", border: `1px solid ${S.border}`, borderRadius: 16, padding: "1rem 1.25rem", marginBottom: "1rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ width: 200, height: 4, background: "var(--surface-2)", borderRadius: 100, overflow: "hidden" }}>
            ...
            <div style={{ background: "var(--surface)", border: `1px solid ${S.border}`, borderRadius: 24, padding: "2rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: "1.5rem" }}>
                {q.options.map((opt, i) => (
                  <button key={i} onClick={() => setSel(i)}
                    style={{ textAlign: "left", padding: "0.85rem 1rem", borderRadius: 14, border: `1px solid ${sel === i ? "rgba(204,255,0,0.5)" : S.border}`, background: sel === i ? "var(--accent-dim)" : "var(--surface-2)", color: sel === i ? "var(--text)" : "var(--text-2)", fontSize: "0.85rem", cursor: "pointer", display: "flex", gap: 10, alignItems: "flex-start", transition: "all 0.12s" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button onClick={next} disabled={sel === null}
                  style={{ display: "flex", alignItems: "center", gap: 6, padding: "0.65rem 1.5rem", background: sel === null ? "var(--surface-2)" : S.accent, color: sel === null ? "var(--text-2)" : "var(--accent-fg)", fontWeight: 700, fontSize: "0.85rem", border: "none", borderRadius: 100, cursor: sel === null ? "not-allowed" : "pointer" }}>
        ...
        {phase === "result" && (
          <div>
            <div style={{ background: "var(--surface)", border: `1px solid ${S.border}`, borderRadius: 24, padding: "3rem 2rem", textAlign: "center", marginBottom: "1.5rem" }}>
              <div style={{ fontSize: "5rem", fontWeight: 900, letterSpacing: "-0.05em", lineHeight: 1, color: passed ? "var(--accent-text)" : "var(--text)", marginBottom: "0.5rem" }}>
              ...
              <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
                <Link href="/modules" style={{ padding: "0.65rem 1.25rem", border: `1px solid ${S.border}`, background: "transparent", color: "var(--text-2)", borderRadius: 100, textDecoration: "none", fontSize: "0.85rem" }}>
                  Module wiederholen
                </Link>
                {passed && (
                  <Link href="/certificate" style={{ display: "flex", alignItems: "center", gap: 6, padding: "0.65rem 1.5rem", background: S.accent, color: "var(--accent-fg)", fontWeight: 700, fontSize: "0.85rem", borderRadius: 100, textDecoration: "none" }}>
