    ...
    <DashboardLayout>
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        <div style={{ marginBottom: "2rem" }}>
          ...
          <h1 style={{ fontSize: "2rem", fontWeight: 700, letterSpacing: "-0.03em" }}>Wissensdatenbank</h1>
        </div>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: "1.5rem" }}>
          {cats.map(c => (
            <button key={c} onClick={() => setCat(c)} style={{
              padding: "0.3rem 0.85rem", borderRadius: 10, fontSize: "0.78rem", fontWeight: 500, cursor: "pointer",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <div key={item.term} style={{ background: "var(--surface)", border: `1px solid ${open === item.term ? "var(--accent-dim3)" : S.border}`, borderRadius: 16, overflow: "hidden", transition: "border-color 0.15s" }}>
              <button onClick={() => setOpen(open === item.term ? null : item.term)}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ fontSize: "0.65rem", color: "var(--text-2)", background: "var(--surface-2)", padding: "0.15rem 0.5rem", borderRadius: 10 }}>{item.category}</span>
                </div>
          ...
          {filtered.length === 0 && (
            <div style={{ textAlign: "center", padding: "4rem", color: "var(--text-2)" }}>
              ...
              Keine Ergebnisse für &laquo;{search}&raquo;
            </div>
          )}
        </div>
      </div>
